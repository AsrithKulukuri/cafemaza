...
export function OrderOnlineClient() {
    ...
    return (
        <div className="relative mx-auto max-w-7xl px-6 pb-20 md:px-10">
            <div className="mb-10">
                <p className="text-sm uppercase tracking-[0.2em] text-[#CFAF63]">Online Food Ordering • Khammam</p>
                <h1 className="mt-2 font-[var(--font-heading)] text-4xl md:text-6xl text-[#F5F5F5] leading-tight">
                    Order Food Online in Khammam
                </h1>
                <div className="mt-3 h-[2px] w-24 bg-gradient-to-r from-[#CFAF63] to-transparent" />
                <p className="mt-4 text-[#F5F5F5]/70 max-w-2xl text-sm md:text-base leading-relaxed">
                ...
                <div className="mt-4 flex items-center gap-2 text-xs font-mono text-[#CFAF63]">
                    <MapPin className="h-3.5 w-3.5" /> Fast Delivery Zone: Khammam, V.Venkatayapalem (Telangana 507318)
                </div>
            </div>
                                    ...
                                    <li key={item.key ?? item.name} className="rounded-xl border border-[#CFAF63]/15 bg-[#141414] p-3">
                                        <div className="flex items-start justify-between gap-2">
                                            <div>
                                                ...
                                                {item.selectedVariant ? <p className="text-xs text-[#999]">{item.selectedVariant.name}</p> : null}
                                            </div>
                                            <button onClick={() => removeItem(item.key)} className="text-[#FF6A00] hover:text-[#FF8A3D]">
                                                <Trash2 size={14} />
                                            </button>
                                        </div>
                                        <div className="mt-3 flex items-center justify-between">
                                            <div className="inline-flex items-center rounded-full border border-[#CFAF63]/25">
                                                <button onClick={() => decreaseQty(item.key)} className="px-3 py-1 text-[#F5F5F5]/75 hover:text-[#F5F5F5]"><Minus size={13} /></button>
                                                <span className="min-w-8 text-center text-xs text-[#CFAF63]">{item.qty}</span>
                                                <button onClick={() => increaseQty(item.key)} className="px-3 py-1 text-[#F5F5F5]/75 hover:text-[#F5F5F5]"><Plus size={13} /></button>
                                            </div>
                                            <span className="text-sm text-[#CFAF63]">₹{(item.selectedVariant?.price ?? item.price ?? 0) * item.qty}</span>
                                        </div>
                                    </li>
                                ))
                            ) : (
