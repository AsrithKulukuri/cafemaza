...
export default function TakeawayPage() {
    return (
        <div className="mx-auto max-w-6xl px-6 pb-20 md:px-10 space-y-10">
            <SectionReveal className="glass-card rounded-3xl border border-[#CFAF63]/25 p-8">
                <p className="text-sm uppercase tracking-[0.2em] text-[#CFAF63]">Takeaway &amp; Parcel Counter</p>
                <h1 className="mt-2 font-(--font-heading) text-4xl md:text-5xl text-[#F5F5F5]">
                ...
                <div className="mt-6 flex flex-wrap items-center gap-4">
                    <a
                        ...
                        className="inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-[#CFAF63] to-[#FF6A00] px-6 py-3 text-xs font-bold text-black hover:opacity-90 transition"
                    >
                        <Phone className="h-4 w-4" /> Call for Quick Pickup (+91 99596 91599)
                    </a>
                    <Link
                        href="/order-online"
                        className="inline-flex items-center gap-2 rounded-full border border-[#CFAF63]/40 bg-zinc-900 px-6 py-3 text-xs font-semibold text-[#F5F5F5] hover:border-[#CFAF63] transition"
                    >
            ...
            <SectionReveal className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
                {menuCategories.map((category) => (
                    <article key={category.id} className="glass-card rounded-2xl border border-[#CFAF63]/20 p-5">
                        ...
                        <p className="mt-2 text-sm text-[#F5F5F5]/70">{category.items.length} dishes available for fresh parcel &amp; takeaway in Khammam.</p>
                        <Link href={`/menu`} className="mt-4 inline-block text-xs font-semibold text-[#CFAF63] hover:underline">
