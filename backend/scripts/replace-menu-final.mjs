...
// New menu data exactly as provided
const newMenuData = [
...
// Determine veg/non-veg based on explicit names, categories, and keywords.
const vegCategories = new Set([
    'Desserts & Sweets',
    'Indian Breads',
    'Mocktails',
    'Veg Soups',
...
const vegNameOverrides = new Set([
    'aalu parata',
    'babycorn crispy',
    'biryani rice',
    'curd rice',
...
const nonVegNameOverrides = new Set([
    'american chopsy with sauce (nonveg)',
    'bbq setup',
    'cafe maza special stroganoff (non-veg)',
    'chelo kebab ( mutton & chicken )',
    'chinese non veg platter',
    'extra dum 1 pic',
    'italian soup (non-veg)',
    'marag shorba',
    'mix fried rice',
    'mix hakka noodles',
    'paya shorba',
    ...
    'sangam seek kebab',
    'sholey kebab',
    'singapore noodles (non-veg)',
    'special tangdi biryani',
    'tangdi kabab',
    'tandoori mix non veg platter',
    'tandoori non veg platter ( 5 items )',
]);
...
function determineVeg(item) {
    ...
    if (nonVegNameOverrides.has(lower)) return false;
    if (vegNameOverrides.has(lower)) return true;
    if (vegCategories.has(item.category)) return true;
    if (nonVegCategories.has(item.category)) return false;
...
async function replaceMenu() {
    try {
        await mongoose.connect(process.env.MONGODB_URI || 'mongodb://127.0.0.1:27017/cafe_maza');
        console.log('✓ Connected to MongoDB');

        // Delete all existing menu items
        const deleteResult = await MenuItem.deleteMany({});
        console.log(`✓ Deleted ${deleteResult.deletedCount} old menu items`);

        // Prepare new items with veg/non-veg determination
        const itemsToInsert = newMenuData.map(item => ({
            ...item,
            isVeg: determineVeg(item),
            category: item.category
        }));

        // Insert new menu items
        const insertResult = await MenuItem.insertMany(itemsToInsert);
        ...
        console.log(`\n✅ Menu replacement complete! Total: ${insertResult.length} items`);
        process.exit(0);
    } catch (error) {
