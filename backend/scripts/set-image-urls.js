...
// Direct Unsplash image URLs (no API auth needed, CDN delivery)
...
async function updateMenuWithImages() {
    try {
        ...
        console.log(`Updating ${items.length} menu items with image URLs...\n`);
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            if (!imageUrl) {
                console.log(
                    `⚠ [${i + 1}/${items.length}] ${item.name} - no image mapping`
                );
                skipCount++;
                continue;
            }
            ...
            // Update in database
            await MenuItem.findByIdAndUpdate(item._id, { image: imageUrl });
            console.log(`  ✓ Updated with Unsplash image URL`);
            successCount++;
        }
