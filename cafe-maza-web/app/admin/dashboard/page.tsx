...
export default function AdminDashboard() {
    ...
    function getSafeMenuImageSrc(src: string | undefined) {
        ...
        const baseSrc = `${src}${separator}${cacheBuster}`;

        if (src.includes("images.unsplash.com")) {
            return `${baseSrc}&auto=format&fit=crop&w=640&q=55`;
        }
                        ...
                        {showNewMenuForm && (
                            ...
                            >
                                ...
                                <div className="flex gap-3 mt-4">
                                    ...
                                    >
                                        {menuSaving ? "Saving..." : editingMenuItemId ? "Update Item" : "Save Item"}
                                    </button>
                                    <button
                                        onClick={() => {
                                            setShowNewMenuForm(false);
                                    ...
                                    {menuItems.map((item, index) => (
                                        <tr key={`${item._id || `menu-item-${item.name}`}-${index}`} className="border-b border-[#1A1A1A] align-top">
                                            ...
                                            <td className="py-3 px-3 text-[#CCC]">
                                                {((item.categories && item.categories.length > 0) ? item.categories : item.category ? [item.category] : []).join(", ")}
                                            </td>
                                            <td className="py-3 px-3 text-[#CFAF63]">
                                                {item.variants && item.variants.length > 0 ? (
                                                    <div className="space-y-1 text-xs">
                                                        {item.variants.map((variant, variantIndex) => (
                                                            <div key={`${item._id}-${variant.name}-${variantIndex}`}>{variant.name} ₹{variant.price}</div>
                                                        ))}
                                                    </div>
                                                ) : item.price != null ? (
                                                    `₹${item.price}`
                                            ...
                                            <td className="py-3 px-3">
                                                <div className="flex flex-wrap gap-1.5">
                                                    ...
                                                    {item.isSpecial ? <span className="rounded-full bg-[#CFAF63]/20 px-2 py-0.5 text-xs text-[#CFAF63]">Recommended</span> : null}
